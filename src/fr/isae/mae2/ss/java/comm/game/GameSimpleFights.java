...
public class GameSimpleFights implements Callable<Boolean> {
	...
	public GameSimpleFights(Socket socket) throws IOException {
		...
		Thread.currentThread().setName("Thread-" + getClass().getSimpleName() + "-Against-" + against);
	}
	...
	private int computeNextUnit() {
		...
		// Note: do that at the end, correct the other FIXMEs first
	...
	public Boolean call() throws IOException {
		// list of battle results for the 3 possible outcomes:
		// 0:draws, 1:successes, 2:failures
		...
		// (in fact, increment the number for the outcomes)
	...
	/**
	 * Main method. Change the arguments by going to Run > Run configuration > Arguments
	 *
	 * @param args The arguments. If empty, runs the game locally.
	 * 	If "server", server mode, otherwise client mode.
	...
	public static void main(String[] args) throws IOException {
		if (args.length == 0) {
			...
			// a server
			// FIXME : create a server like above,
			// with an infinite loop so that you can receive other players
		} else {
			// a client
			// FIXME : create a client like above,
			// that connects to the
			// computer named args[0] instead of localhost
		}
	}
}
